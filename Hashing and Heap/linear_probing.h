...
// collisions ()         - > To find the number of collisions
...
class HashTableLinear {
 public:
  ...
  void setCollision(){   // to reset/set collision
    number_Of_Collision = 0 ;
 ...
 private:        
  ...
  size_t FindPos(const HashedObj & x) const {
    ...
    while (array_[current_pos].info_ != EMPTY &&
	   array_[current_pos].element_ != x) {
      number_Of_Collision++; // increment number of collisions
      current_pos += offset;  // Compute ith probe.
      // offset +=2; Deleted because for linear probing we only need to increase by +1, which we doing above while increasing current_pos
